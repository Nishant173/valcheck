...
class Item(base_validator.BaseValidator):
    name = fields.ChoiceField(
        choices=list(ITEM_NAMES),
    ...
    def model_validator(self):
        ...
        if unit == 'count' and not isinstance(quantity, int):
            ...
            return models.Error(details={"message": message})
        return None


class Cart(base_validator.BaseValidator):
    customer_name = fields.StringField(allow_empty=False)
    timestamp_of_purchase = fields.DatetimeStringField()
    items = fields.ListOfModelsField(model=Item, allow_empty=False)
    additional_info = fields.AnyField(required=False, nullable=True, default_func=lambda: None)
    ...
    def model_validator(self):
        ...
        if 17 <= hour_of_day < 22 and num_items < 3:
            message = f"We expect you to add atleast 3 items while shopping between 5pm - 10pm. Received only {num_items} items"
            return models.Error(details={"message": message})
        return None
